...
const handleDeleteDocument = (index) => {
  ...
  const deletedDocument = updatedDocuments.splice(index, 1)[0]; // Remove and store deleted document
  setNewDocument(updatedDocuments);

  // Update data.json (local storage) as well
  localStorage.setItem('data', JSON.stringify(updatedDocuments));

  // If the selected document is deleted, clear the selectedDocument
...
const getEditedContent = (content) => {
  setSelectedDocument((prev) => ({
    ...prev,
    content: content, // Update the content of selectedDocument
  }));
};
